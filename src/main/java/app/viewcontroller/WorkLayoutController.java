...
public class WorkLayoutController {
  ...
  public void initialize() {
    ...
    Arrays.stream(LootType.values()).forEach(value -> tableMap.put(value, makeTab(value)));
  }

  private TableView<Loot> makeTab(LootType type) {
    // create tab
    final Tab tab = new Tab();
    tab.setText(type.toString());

    // create vbox to hold button pane and tableview
    VBox vbox = new VBox();
    vbox.setSpacing(5);
    vbox.setPadding(new Insets(10, 10, 10, 10));

    // create button pane and tableview
    TableView<Loot> table = makeTable();
    final ButtonBar buttonBar = makeButtonBar(table);

    // add items to vbox
    vbox.getChildren().addAll(buttonBar, table);
  ...
  private ButtonBar makeButtonBar(TableView<Loot> table) {
  ...
  private TableView<Loot> makeTable() {
